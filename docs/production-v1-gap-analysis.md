...
| Schema drift (test entities) | ⚠️ | `SchemaValidator` с ограничениями |
| Lifecycle / test hygiene | ❌ | `ScenarioContext.clear()` только в README, нет extension |
| Testcontainers | ❌ | Не в зависимостях, нет примеров |
| Reporting (Allure) | ❌ | В roadmap, `ResourceUsageMonitor` без интеграции |
| Adaptation templates | ⚠️ | Playbook в AGENTS.md, нет `application-staging.yml` примера |
| Docker runner warmup | ⚠️ | Dockerfile копирует не все `module-*/build.gradle` |
| Contract depth | ⚠️ | Минимальный DSL; JSON Schema — задокументированный swap |
| Data pools | ❌ | В roadmap |
| Distributed tracing / correlation | ⚠️ | Scope-id — ручная адаптация, нет генератора/хука |
...
|---|-----|----------|--------------|
| 1 | **JUnit extension для очистки контекста** | `ScenarioContext` и `RunUniqueValues` могут протекать между тестами на одном worker thread | `ScenarioContextExtension` в `core`: `@AfterEach` → `ScenarioContext.clear()`; опционально reset unique registry per test class |
...
- [ ] `./gradlew build` green без внешних сервисов
- [ ] Новый адаптер проходит `docs/adaptation-checklist.md` без «скрытых» шагов
- [ ] `ScenarioContext` не протекает между тестами в parallel run
- [ ] Есть рабочий пример test scope → mock isolation → REST call
- [ ] Docker runner image build проходит с полным набором модулей
- [ ] Документирована связка с Testcontainers и Pact (этот файл + README anchor)
- [ ] example-tests README соответствует все тест-классам
